...
function MyBookingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchMyBookings = async () => {
      ...
      try {
        ...
        const response = await axios.get<BookingResDto[]>('https://ticket-distribute-ver.onrender.com/api/bookings/my');
  ...
  const handleCancelBooking = async (bookingId: number) => {
    ...
    try {   //2. (중요!)백엔드 취소 API호출
      ...
      await axios.delete(`https://ticket-distribute-ver.onrender.com/api/bookings/${bookingId}`);
          ...
          {bookings.map((booking) => {
            ...
            return (
              <div key={booking.bookingId} className={cardClass}>
                ...
                {/* 1. 왼쪽 (포스터) */}
                <img 
                  src={`https://ticket-distribute-ver.onrender.com${booking.performance.posterImageUrl}`}
